...
The only difference between Strict privacy and Privacy is the number of corrupted voters. In Strict privacy games \textbf{all} voters are corrupted and  $\mathcal{A}$ votes on their behalf. On the contrary, in the Privacy games at most $t$ voters are corrupted and all other are honest. The crucial moment is that honest voters are able to verify that their vote was cast as intended, but an adversary can not since he does not know the intent and does not control all entities. Therefore, voters may lie about vote while an adversary \fix{have no ways}{has no way} to learn the truth and not to win the game $G_{t-priv,<honest~entities>}^{\mathcal{A}, Sim}(1^{\lambda})$. \\

 The specification of trusted entities splits the Voter Privacy into different scenarios. All meaningful cases of collusion fall into two scenarios: (1) EA and T are honest and (2) VSD and T are honest. It is possible to construct e-voting scheme, that is private with respect to only trusted VSD, however, most known schemes require trusted T as well.\fix{}{elaborate more on this statement} \\
...
The game $G_{strict, \vsd,\T}^{\mathcal{A},Sim}(1^{\lambda})$ is defined as follows:
\begin{enumerate} 
...
\begin{definition}[Strict privacy: $\vsd$ and $\T$ are honest]
The e-voting system $\Pi$ achieves strict voter privacy in case when $\vsd$ and $\T$ are trusted, if there is a PPT simulator $Sim$ such that for any PPT adversary $\mathcal{A}$:\\\\
 $|\Pr[G_{strict,\vsd,\T}^{\mathcal{A},Sim}(1^{\lambda},n,m) = 1] - \frac{1}{2}| = negl(\lambda)$
 \end{definition}
 %
%=============================================================================================================
%
\section{Strict Privacy contradicts E2E Verifiability. }
As we discuss in section \ref{demospriv}, \textit{Demos Privacy} \emph{implies}{is no stronger than} the weaker security than \textit{Voter Privacy}, therefore the same holds for strict privacy notion. However, we use our Strict Privacy definition instead of strict version of Demos Privacy to prove that "Any system $\Pi$ that satisfies the definition of strict privacy, is "receipt free" but not E2E Verifiable", because we need privacy guarantees for the different tallies case. As for E2E Verifiability, we use the ideal functionality designed by V. Teague, A. Kiayias, T. Zacharias et al. \cite{idfunc} that captures the E2E Verifiability definition by Kiayias et. al.  The paper, where the ideal functionality for end-to-end verifiable elections is defined, currently is under submission, therefore we do not list the functionality here.\\
...
\begin{theorem}[Strict Privacy contradicts E2E Verifiability] 
...
\subsection{Part 1: E2E Verifiability vs Strict privacy with respect to $\ea$ and $\T$.}
...
\end{minipage}}
\\\\
\fix{}{An adversary for the E2E verifiability game}
\\
\fbox{\begin{minipage}{30em}
$\mathcal{A}$:\\
--  defines an election setup information:\\ $election\_params = (\mathcal{C},\mathcal{V}, \mathcal{U}, params)$, where $\mathcal{C}$ - list of candidates, $\mathcal{V}$ - list of voters, $\mathcal{U}$ - list of allowed candidates' selections, $params$ - other required information.\\
...
\subsection{Part 2: E2E Verifiability vs Strict privacy with respect to $\vsd$ and $\T$.}
