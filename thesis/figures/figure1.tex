...
\begin{document}
  ...
  \node[draw,rectangle]  at (6, 10) (ea)[sensor] {$\ea$};
   ...
   \node [dashed, draw,circle] at (8,10) (v)[v]{$V_i$};
  
  \node[draw,rectangle]  at (-2, 5) (vsd)[sensor] {$\vsd$};
    \node[draw,rectangle]  at (4, 9) (tr)[sensor] {$T$};
      \node[draw,rectangle]  at (-2, 7) (vc)[sensor] {$\voc$};
  \node[draw,rectangle,text width=6.7cm]  at (6, 5) (c) {$a \leftarrow \{0,1\} $  \\ $\tilde{s_i} \leftarrow Sim(s_i,U_i^0,U_i^1,V_i)$ \\ if $a = 0$ then  $s_i^0 = \tilde{s_i}, s_i^1 = s_i$\\ else if $a=1$, $s_i^0=s_i, s_i^1 = \tilde{s_i}$  };
   ...
   \draw[->,shorten <=0.5cm] (v) node[font=\small, above,xshift=2cm,yshift =1.5 cm] {$2)$ Registration} |- (ea);
   \draw[transform canvas={yshift=-1ex},->,shorten >=0.5cm] (c) node[font=\small, above,xshift=-5cm]  {$4) (s_i^0, s_i^1)$}  -- (vsd);
