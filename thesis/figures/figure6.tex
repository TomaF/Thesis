...
\begin{document}
  ...
  \node[draw,rectangle]  at (6, 8) (ea)[sensor] {$\ea$};
  \node [draw,circle] at (-2,8.5) (v)[v]{$V_i$};
  
  \node[draw,rectangle]  at (-2, 5) (vsd)[sensor] {$\vsd$};
    \node[draw,rectangle]  at (4, 8) (tr)[sensor] {$T$};
      \node[draw,rectangle]  at (-2, 7) (vc)[sensor] {$\voc$};
  \node[draw,rectangle,text width=5cm]  at (6, 5) (c) {$a \leftarrow \{0,1\} $  \\ $\tilde{s_i} \leftarrow Sim(s_i,U_i^0,U_i^1,V_i)$ \\ if $a = 0$ then  $s_i^0 = \tilde{s_i}, s_i^1 = s_i$\\ else if $a=1$, $s_i^0=s_i, s_i^1 = \tilde{s_i}$  };
   \node[draw,rectangle, fill =  gray]  at (1, 7) (bb)[sensor] { BB};
   \path (vc.south) +(-0,-0.5) node (a) {\Huge{$\mathcal{A}$}};
   \draw[transform canvas={xshift=-2ex},->,shorten >=0.4cm] (ea) node[font=\small, below,xshift=-2cm,yshift = -1 cm]  {6) $b_i$}  |- (bb);
   
   \draw[transform canvas={yshift=-1ex},->,shorten <=0.5cm]  (vsd) node[font=\small, above,xshift=2.7cm] {$2)$ Registration} -- (c);
   \draw[transform canvas={yshift=-3ex},->,shorten >=0.5cm] (c) node[font=\small, below,xshift=-5cm]  {$4) (s_i^0, s_i^1)$}  -- (vsd);
    ...
    \draw[->,shorten <=0.5cm] (vc) node[font=\small, above,xshift=1.7cm] {$5) b_i$} -- (bb);
      ...
      \draw[->] (tr) node[font=\small, above,xshift=-1.5cm] {$7)  \tau $} -| (bb);
    
   \draw[<-] (c) node[font=\small, right,yshift=2cm] {$3) s_i$} -- (ea);
    ...
    \end{pgfonlayer}
       ...
       \begin{pgfonlayer}{background}
        ...
        \path[rounded corners, draw=black, fill = gray!10]
            (a) rectangle (b);
    \end{pgfonlayer}
    \path (c.north) +(+2,1) node (RE) {\Huge{$\mathcal{C}$}};
  \end{tikzpicture}
\end{document}