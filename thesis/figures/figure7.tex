...
\begin{document}
 \begin{tikzpicture}[thick]
  ...
  \node[draw,rectangle,text width=5.5cm]  at (5.2, 5) (c) {$a \leftarrow \{0,1\} $ \\  $(\tilde{b_i}, \tilde{r_i}) \leftarrow Sim(s_i,U_{i_1},U_{i_2}, V_i)$\\If $a=0$, then:\\ $(b_0, r_0) = (\tilde{b_i},\tilde{r_i})$\\ $(b_1,r_1)= (b_i,r_i)$ \\ Else if $a = 1$ \\$(b_0,r_0) = (b_i,r_i) $ \\$(b_1, r_1) = (\tilde{b_i},\tilde{r_i})$};
   ...
   \draw[transform canvas={yshift=-2.7ex},->,shorten >=0.5cm] (c) node[font=\small, below,xshift=-4.7cm]  {$4) (b_0,r_0, b_1,r_1)$}  -- (ea);
      \draw[transform canvas={yshift=2.7ex},<-,shorten >=0.5cm] (c) node[font=\small, above,xshift=-4.7cm]  {$1) U_{i_1},U_{i_2}$}  -- (ea);
   
      \draw[transform canvas={yshift=-0.7ex},<-,shorten >=0.5cm] (c) node[font=\small, above, xshift = -4.7cm] {$2) (s_i,U_{i_1})$} -- (ea);
