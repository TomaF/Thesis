...
\item   \textbf{Result}: The algorithm  \textbf{Result($\tau, sKey$)} performs the decryption and outputs the result $R_{\tau}$ of the election and proofs of correct decryption $\boxed{R_{\tau}}$ or returns  $\perp$in case such result is undefined.
...
The concept of blind signing was introduced by David Chaum in \cite{Chaum1982}. The idea is to obtain a signature for a secret without revealing the secret to a signer. Let's say Alice wants Bob to sign a value x with his private key (d,N) without revealing x. To do so, Alice picks random blinding factor z so gcd(z,N) = 1 and calculates $p=xz^e\bmod N $. She sends p to Bob. Bob signs $p^d\bmod N$ and sends it back to Alice. Alice calculates $p^d\bmod N z^{-1} = x^d\bmod N$ which is a signature for value x. Bob couldn't find out x on his own even if he has a quantum computer since it's multiplied on unknown blinding factor. 
\subsubsection{Homomorphic Integer Commitment and Homomorphic Cryptosystem}
There are only a few homomorphic integer commitment schemes \textcolor{red}{add links}, and they are quite similar in structure. For the blind signature scheme we offer the following variant.  We choose a modulus $n$ as a product of two safe primes and random generators $g_1, \dots, g_k,h$ of $QR_n$. In order to commit to integers $m_1,m_2, \dots, m_k$ using randomness $r$, we compute $c = com( m_1,m_2, \dots, m_k; r) = g_1^{m_1}g_2^{m_2}\dots g_k^{m_k}h^r$. The commitment is statistically hiding if the randomness choice $r \leftarrow \{0,1\}^{l_r}$.\\

Root extracting property: When proving soundness and knowledge in our protocols we need a root extraction property, which says that if a ciphertext raised to a nontrivial exponent encrypts 1, then the ciphertext itself encrypts 1 \cite{Groth2010}. For the homomorphic encryption generalisation of this property can be formalized as follows \cite{Groth2005}: \\
...
ElGamal encryption is semantically secure, have the root extraction property and admit threshold decryption  \cite{Groth2010}. 
\subsubsection{Proving blind signature knowledge}
\fbox{\begin{minipage}{30em}
\textbf{Argument:}\\
$\sigma = H(x)^d\bmod N$\\
Choose random $A \in _R Z_N$\\
$R = A^e\bmod N$\\
$c = \hat{H}(Encr(vote)||\dots||R)$\\
$S = A\sigma^c\bmod N$\\
Proofs: $(c,S)$\\
\textbf{Verify:}\\
$\hat{R} = \frac{S^e}{H(x)^c}\bmod N$\\
$\hat{c} = \hat{H}(Encr(vote)||\dots||\hat{R})$\\
Check that $\hat{c} = c$
\end{minipage}}\\
if $\hat{c} = \hat{H}(Encr(vote)||\dots|| \frac{S^e}{H(x)^c}\bmod N )= \hat{H}(Encr(vote)||\dots|| A^e\bmod N ) =  \hat{H}(Encr(vote)||\dots||R) = c$ then proofs of signature knowledge are valid.
